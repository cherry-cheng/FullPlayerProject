package com.example.cherry.fullplayer.view;
...
public class WaveSideBarView extends View {
    ...
    private boolean isInAnimator = false;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mWaveColor = Color.parseColor("#00CFA1");
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        final float y = event.getY() - mPadding;
        final float x = event.getX();

        oldChoose = mChoose;
        newChoose = (int) (y / (mHeight - 2 * mPadding) * mLetters.size());
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mCenterY = (int) y + mPadding;
                if (y >= 0 && y <= (mHeight - 2 * mPadding)) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mItemHeight = (mHeight - 2 * mPadding) / mLetters.size();
